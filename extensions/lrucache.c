...
stx_cache_t* stx_cache_create(size_t max_size, size_t max_weight, size_t hash_size,
    ...
    newcache->table = (stx_cache_entry_t**)calloc(hash_size, sizeof(stx_cache_entry_t*));
...
void stx_cache_traverse(stx_cache_t* cache, void (*callback)(void* key, void* data)) {
    size_t i;
...
void stx_cache_traverse_lru(stx_cache_t* cache, void (*callback)(void* key, void* data), unsigned int n) {
    stx_clist_t* q;
    stx_cache_entry_t* entry;
    for (q = STX_CLIST_HEAD(&cache->lru_list); q != &cache->lru_list && n; q = q->next, n--) {
        entry = STX_CACHE_ENTRY_PTR(q);
        (*callback)(entry->key, entry->data);
    }
}

void stx_cache_traverse_mru(stx_cache_t* cache, void (*callback)(void* key, void* data), unsigned int n) {
    stx_clist_t* q;
    stx_cache_entry_t* entry;
    for (q = STX_CLIST_TAIL(&cache->lru_list); q != &cache->lru_list && n; q = q->prev, n--) {
        entry = STX_CACHE_ENTRY_PTR(q);
        (*callback)(entry->key, entry->data);
    }
}
...
// Cache entry methods
stx_cache_entry_t* stx_cache_entry_create(void* key, void* data, size_t weight) {
...
int stx_cache_entry_insert(stx_cache_t* cache, stx_cache_entry_t* entry) {
    ...
    // If cache capacity is exceeded, try to remove LRU entries till there is enough room or LRU list is empty.
    ...
    for (old_entry = cache->table[bucket]; old_entry; old_entry = old_entry->next) {
        if (!old_entry->delete_pending && cache->key_cmp_fn(entry->key, old_entry->key) == 0) {
            break;
        }
    }
    if (old_entry) {
