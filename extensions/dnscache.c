...
extern int _stx_dns_getaddrlist(const char* hostname, struct in_addr* addrs, int* num_addrs, st_utime_t timeout);
...
static int lookup_entry(const char* host, struct in_addr* addrs, int* num_addrs, int rotate) {
...
static void insert_entry(const char* host, struct in_addr* addrs, int count) {
    ...
    if (_stx_dns_ttl > 0) {
        ...
        entry = stx_cache_entry_create(key, data, strlen(host) + 1 + sizeof(stx_dns_data_t) + n + stx_cache_entry_sizeof());
        if (key && data && data->addrs && entry && stx_cache_entry_insert(_stx_dns_cache, entry) == 0) {
            stx_cache_entry_release(_stx_dns_cache, entry);
            return;
        }
...
int _stx_dns_cache_getaddrlist(const char* hostname, struct in_addr* addrs, int* num_addrs, st_utime_t timeout, int rotate) {
    char host[128];
    int n, count;
...
int stx_dns_cache_init(size_t max_size, size_t max_bytes, size_t hash_size) {
    _stx_dns_cache = stx_cache_create(max_size, max_bytes, hash_size, hash_hostname, (long (*)(const void*, const void*))strcmp, cleanup_entry);
    if (!_stx_dns_cache) {
        return -1;
    }
    return 0;
